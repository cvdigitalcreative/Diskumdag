...
class M_pengumuman extends CI_Model{
	...
	function get_all_pengumuman_by_id(){
		$hsl=$this->db->query("SELECT tbl_pengumuman.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_pengumuman ORDER BY tulisan_id DESC");
		return $hsl;
	}

	function get_all_pengumuman(){
		$hsl=$this->db->query("SELECT tbl_pengumuman.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_pengumuman ORDER BY tanggal_pengumuman DESC");
		return $hsl;
	}

	function get_join_table1($slug){
		$hsl=$this->db->query("SELECT * FROM tbl_pengumuman JOIN tbl_album_pengumuman ON tbl_pengumuman.tulisan_id=tbl_album_pengumuman.album_kategori_id WHERE tbl_pengumuman.tulisan_slug='$slug'");
		return $hsl;
	}

	function get_join_table($slug){
		$hsl=$this->db->query("SELECT DISTINCT tulisan_judul,tanggal_pengumuman,album_gambar FROM tbl_pengumuman JOIN tbl_album_pengumuman ON tbl_pengumuman.tulisan_id=tbl_album_pengumuman.album_kategori_id WHERE tbl_pengumuman.tulisan_slug='$slug'");
		return $hsl;
	}

	function get_pengumuman_by_slug($kode){
		$hsl=$this->db->query("SELECT tbl_pengumuman.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_pengumuman where tulisan_slug='$kode'");
		return $hsl;
	}

	function get_all_pengumuman_by_tanggal(){
		$hsl=$this->db->query("SELECT tbl_pengumuman.*,DATE_FORMAT(tanggal_pengumuman,'%d/%m/%Y') AS tanggal FROM tbl_pengumuman ORDER BY tanggal_pengumuman DESC limit 3 ");
		return $hsl;
	}

	function simpan_pengumuman($tanggal,$judul,$isi,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("INSERT into tbl_pengumuman(tanggal_pengumuman,tulisan_judul,tulisan_isi,tulisan_pengguna_id,tulisan_author,tulisan_gambar,tulisan_slug) values ('$tanggal','$judul','$isi','$user_id','$user_nama','$gambar','$slug')");
		return $hsl;
	}
	function simpan_pengumuman_tanpa_gambar($tanggal,$judul,$isi,$user_id,$user_nama,$slug){
		$hsl=$this->db->query("INSERT into tbl_pengumuman(tanggal_pengumuman,tulisan_judul,tulisan_isi,tulisan_pengguna_id,tulisan_author,tulisan_slug) values ('$tanggal','$judul','$isi','$user_id','$user_nama','$slug')");
		return $hsl;
	}
	function get_tulisan_by_kode($kode){
		$hsl=$this->db->query("SELECT tbl_pengumuman.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_pengumuman where tulisan_id='$kode'");
		return $hsl;
	}
	function update_tulisan($tulisan_id,$tanggal,$judul,$isi,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("UPDATE tbl_pengumuman set tanggal_pengumuman='$tanggal',tulisan_judul='$judul',tulisan_isi='$isi',tulisan_pengguna_id='$user_id',tulisan_author='$user_nama',tulisan_gambar='$gambar',tulisan_slug='$slug' where tulisan_id='$tulisan_id'");
		return $hsl;
	}
	function update_tulisan_tanpa_img($tulisan_id,$tanggal,$judul,$isi,$user_id,$user_nama,$slug){
		$hsl=$this->db->query("UPDATE tbl_pengumuman set tanggal_pengumuman='$tanggal',tulisan_judul='$judul',tulisan_isi='$isi',tulisan_pengguna_id='$user_id',tulisan_author='$user_nama',tulisan_slug='$slug' where tulisan_id='$tulisan_id'");
		return $hsl;
	}
	function hapus_tulisan($kode){
		$hsl=$this->db->query("DELETE from tbl_pengumuman where tulisan_id='$kode'");
		return $hsl;
	}
	function get_link_before($link){
		$hsl=$this->db->query("SELECT * FROM tbl_pengumuman WHERE tulisan_id BETWEEN ($link-1) AND $link ");		
		return $hsl;
	}
	function get_link_after($link){
		$hsl=$this->db->query("SELECT * FROM tbl_pengumuman WHERE tulisan_id>$link");		
		return $hsl;
	}
